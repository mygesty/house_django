...
zone_map = {"tianhe": "天河", "baiyun": "白云", "panyu": "番禺", "haizhu": "海珠", "yuexiu": "越秀",
            "huadu": "花都", "liwan": "荔湾", "zengcheng": "增城", "huangpu": "黄埔", "nansha": "南沙", "conghua": "从化"}

attr_zone = ["天河", "白云", "番禺", "海珠", "越秀", "花都", "荔湾", "增城", "黄埔", "南沙", "从化"]

room_map = {"yishi": "一室", "ershi": "二室", "sanshi": "三室", "sishi": "四室", "wushi": "四室以上"}
...
def index(request):
    house_num = Zufang.objects.count()
    averge_price = int(Zufang.objects.all().aggregate(Avg('price'))['price__avg'])
    averge_area = int(Zufang.objects.all().aggregate(Avg('housearea'))['housearea__avg'])
    template = loader.get_template('houseshow/houseshow.html')

    attr_zone = ["天河", "白云", "番禺", "海珠", "越秀", "花都", "荔湾", "增城", "黄埔", "南沙", "从化"]
    values_zone = [Zufang.objects.filter(zone=i).count() for i in attr_zone]
    width = 600
    pie_zone = pie(attr_zone, values_zone, width, ["61%", "50%"])    # 显示房子在各区的分布情况，比例

    # attr_housetype = ["一室", "两室", "三室", "四室", "四室以上"]
    housetype = ["1室", "2室", "3室", "4室"]
    values_housetype = [Zufang.objects.filter(housetype__regex="^\d+室").filter(housetype__contains=i).count() for i in housetype]
    values_housetype.append(Zufang.objects.filter(housetype__regex="^\d+室").count()-sum(values_housetype))
    pie_housetype = pie(attr_housetype, values_housetype, 600, ["61%", "50%"])     # 显示各户型的比例组成

    attr_area = ["50㎡以下", "50-100㎡", "100-150㎡", "150-200㎡", "200-300㎡", "300㎡以上"]
    area = [(50, 100), (100, 150), (150, 200), (200, 300)]
    values_area = [Zufang.objects.filter(housearea__lt=i[1]).filter(housearea__gte=i[0]).count() for i in area]
    values_area.insert(0, Zufang.objects.filter(housearea__lt=50).count())
    values_area.append(Zufang.objects.filter(housearea__gte=300).count())
    ...
    context = dict(
        ...
        myechart2=gzmap.render_embed(),
        host=REMOTE_HOST,
        script_list=gzmap.get_js_dependencies(),
        house_num=house_num,
...
def path_resolute(path: str)->list:
    ...
    return [zone, price, area, housetype]


def multiploy(house_args: list):
            ...
            if len(house_args[2]) != 0:
                if len(house_args[3]) != 0:    # 选择了区域、户型、价格、面积
                    ...
                    if room_map2[house_args[3]] == "四室以上":
                        values = [Zufang.objects.filter(zone=zone_map[house_args[0]]).filter(housenum__gt=4).filter(housearea__gte=int(house_args[2][0])).filter(housearea__lt=int(house_args[2][1])).filter(price__gte=int(house_args[1][0])).filter(price__lt=int(house_args[1][1])).filter(per_price__gte=int(i)).filter(per_price__lt       =int(i)+10).count() for i in attr]
                    else:
                        values = [Zufang.objects.filter(zone=zone_map[house_args[0]]).filter(housenum=room_map2[house_args[3]]).filter(housearea__gte=int(house_args[2][0])).filter(housearea__lt=int(house_args[2][1])).filter(price__gte=int(house_args[1][0])).filter(price__lt=int(house_args[1][1])).filter(per_price__gte=int(i)).filter(per_price__lt=int(i)+10).count() for i in attr]
                    ...
                    if room_map2[house_args[3]] == "四室以上":
                        values_price = [Zufang.objects.filter(zone=zone_map[house_args[0]]).filter(housenum__gt=4).filter(housearea__gte=int(house_args[2][0])).filter(housearea__lt=int(house_args[2][1])).filter(price__gte=int(i)).filter(price__lt=int(i)+50).count() for i in attr_interval_price]
                    else:
                        values_price = [Zufang.objects.filter(zone=zone_map[house_args[0]]).filter(housenum=room_map2[house_args[3]]).filter(housearea__gte=int(house_args[2][0])).filter(housearea__lt =int(house_args[2][1])).filter(price__gte=int(i)).filter(price__lt=int(i)+50).count() for i in attr_interval_price]
                    ...
                    values = [
                        Zufang.objects.filter(zone=zone_map[house_args[0]]).filter(housearea__gte=int(house_args[2][0])).filter(
                    ...
                    values_price = [
                        Zufang.objects.filter(zone=zone_map[house_args[0]]).filter(
                    ...
                    values_price.append(Zufang.objects.filter(zone=zone_map[house_args[0]]).filter(
                ...
                if len(house_args[3]) != 0:    # 选择了区域、价格、户型，没选面积
                    ...
                    value_price_along_housetype = [[Zufang.objects.filter(zone=zone_map[house_args[0]]).filter(
                    ...
                    value_price_along_housetype.append([Zufang.objects.filter(zone=zone_map[house_args[0]]).filter(
                        price__gte=house_args[1][0]).filter(price__lt=int(house_args[1][1])).filter(
                        housearea__gte=i[0]).filter(housearea__lt=i[1]).filter(
                    ...
                    for idx, value in enumerate(value_price_along_housetype):
                        ...
                        value_price_along_area = [[Zufang.objects.filter(zone=zone_map[house_args[0]]).filter(
            ...
            if len(house_args[2]) != 0:
                if len(house_args[3]) != 0:    # 选了区域、面积、户型，没选价格
                    ...
                    values_area = [
                        ...
                        if i != "四室以上"]
                    values_area.append(Zufang.objects.filter(zone=zone_map[house_args[0]]).filter(
                    ...
                    for idx, value in enumerate(value_price_along_housetype):
                        ...
                        value_housetype_along_price = [[Zufang.objects.filter(zone=zone_map[house_args[0]]).filter(
                ...
                if len(house_args[3]) != 0:    # 选了区域、户型，没选面积、价格
                    ...
                    for idx, value in enumerate(value_price_along_area):
                        ...
                        value_area_along_price = [[Zufang.objects.filter(zone=zone_map[house_args[0]]).filter(
                            price__gte=j[0]).filter(price__lt=j[1]).filter(
                            housearea__gte=i[0]).filter(housearea__lt=i[1]).filter(
        ...
        if len(house_args[1]) != 0:
            if len(house_args[2]) != 0:
                if len(house_args[3]) != 0:    # 选了价格、面积、户型
                    ...
                    values_zone = [
                        Zufang.objects.filter(zone=i).filter(
                            housenum=room_map2[house_args[3]]).filter(
                            housearea__gte=int(house_args[2][0])).filter(housearea__lt=int(house_args[2][1])).filter(
                        ...
                        for i in attr_zone]
                    bar_zone = bar_ploy("租房区域分布图", attr_zone, values_zone, title_pos="0%")
                    ...
                    values_zone = [
                        ...
                        attr_zone]
                    bar_zone = bar_ploy("租房区域分布图", attr_zone, values_zone, title_pos="51%")

                    values_housetype = [
                        Zufang.objects.filter(housenum=room_map2[i]).filter(
                            price__gte=int(house_args[1][0])).filter(
                            price__lt=int(house_args[1][1])).filter(
                            housearea__gte=int(house_args[2][0])).filter(
                            housearea__lt=int(house_args[2][1])).count() if i != "四室以上" else
                        Zufang.objects.filter(housenum__gt=4).filter(
                            price__gte=int(house_args[1][0])).filter(
                            price__lt=int(house_args[1][1])).filter(
                            housearea__gte=int(house_args[2][0])).filter(
                    ...
                    value_zone_along_housetype = [[Zufang.objects.filter(zone=i).filter(
                        price__gte=house_args[1][0]).filter(price__lt=int(house_args[1][1])).filter(
                        housearea__gte=house_args[2][0]).filter(housearea__lt=house_args[2][1]).filter(
                        housenum=room_map2[j]).count() for i in attr_zone] if j != "四室以上" else
                                                  [Zufang.objects.filter(zone=i).filter(
                                                      price__gte=house_args[1][0]).filter(
                                                      price__lt=int(house_args[1][1])).filter(
                                                      housearea__gte=house_args[2][0]).filter(
                                                      housearea__lt=house_args[2][1]).filter(
                                                      housenum__gt=4).count() for i in attr_zone] for j in room_map2]
                    ...
                    for idx, value in enumerate(value_zone_along_housetype):
                        ...
                        value_housetype_along_zone = [[Zufang.objects.filter(zone=j).filter(
                            price__gte=int(house_args[1][0])).filter(price__lt=int(house_args[1][1])).filter(
                            housearea__gte=int(house_args[2][0])).filter(housearea__lt=int(house_args[2][1])).filter(
                            housenum=room_map2[i]).count() if i != "四室以上" else
                                                       Zufang.objects.filter(zone=j).filter(
                                                           price__gte=int(house_args[1][0])).filter(
                                                           price__lt=int(house_args[1][1])).filter(
                                                           housearea__gte=int(house_args[2][0])).filter(
                                                           housearea__lt=int(house_args[2][1])).filter(
                                                           housenum__gt=4).count()
                                                       for i in room_map2] for j in attr_zone]
                ...
                if len(house_args[3]) != 0:    # 选了价格、户型，没选区域、面积
                    ...
                    values_area = [
                        Zufang.objects.filter(housenum=room_map2[house_args[3]]).filter(
                            price__gte=int(house_args[1][0])).filter(
                            price__lt=int(house_args[1][1])).filter(
                            housearea__gte=i[0]).filter(
                            housearea__lt=i[1]).count() if house_args[3] != "四室以上" else
                    ...
                    value_zone_along_area = [[Zufang.objects.filter(zone=i).filter(
                        price__gte=house_args[1][0]).filter(price__lt=int(house_args[1][1])).filter(
                        housearea__gte=j[0]).filter(housearea__lt=j[1]).filter(
                        housenum=room_map2[house_args[3]]).count() for i in attr_zone] if house_args[3] != "四室以上" else
                                                  [Zufang.objects.filter(zone=i).filter(
                                                      price__gte=house_args[1][0]).filter(
                                                      price__lt=int(house_args[1][1])).filter(
                                                      housearea__gte=j[0]).filter(
                                                      housearea__lt=j[1]).filter(
                                                      housenum__gt=4).count() for i in attr_zone] for j in area_map]
                    ...
                    for idx, value in enumerate(value_zone_along_area):
                        ...
                        value_area_along_zone = [[Zufang.objects.filter(zone=j).filter(
                            price__gte=int(house_args[1][0])).filter(price__lt=int(house_args[1][1])).filter(
                            housearea__gte=i[0]).filter(housearea__lt=i[1]).filter(
                            housenum=room_map2[house_args[3]]).count() if house_args[3] != "四室以上" else
                                                       Zufang.objects.filter(zone=j).filter(
                                                           ...
                                                           housenum__gt=4).count()
                                                       for i in area_map] for j in attr_zone]
                    ...
                    grid = Grid(width=1200, height=600)
                    values_housetype = [
                        Zufang.objects.filter(price__gte=int(house_args[1][0])).
                    ...
                    values_zone = [
                        Zufang.objects.filter(zone=i).filter(
                            price__gte=house_args[1][0]).filter(price__lt=house_args[1][1]).count() for i in attr_zone]
                    bar_zone = bar_ploy("租房区域分布图", attr_zone, values_zone, title_pos="0%")

                    values_area = [
                        Zufang.objects.filter(price__gte=int(house_args[1][0])).filter(
                            price__lt=int(house_args[1][1])).filter(
                ...
                if len(house_args[3]) != 0:    # 选了面积、户型，没选区域、价格
                    ...
                    values_zone = [
                        ...
                        Zufang.objects.filter(zone=i).filter(
                            housearea__gte=int(house_args[2][0])).filter(
                            housearea__lt=int(house_args[2][1])).filter(
                            housenum__gt=4).count() for i in attr_zone]
                    bar_zone = bar_ploy("租房区域分布图", attr_zone, values_zone, title_pos="51%")
                    ...
                    value_zone_along_price = [[Zufang.objects.filter(zone=i).filter(
                        price__gte=j[0]).filter(price__lt=j[1]).filter(
                        housearea__gte=int(house_args[2][0])).filter(
                        housearea__lt=int(house_args[2][1])).filter(
                        housenum=room_map2[house_args[3]]).count() for i in attr_zone] if j != "四室以上" else
                                              [Zufang.objects.filter(zone=i).filter(price__gte=j[0]).filter(
                                                  price__lt=j[1]).filter(housearea__gte=int(house_args[2][0])).filter(
                                                  housearea__lt=int(house_args[2][1])).filter(
                                                  housenum__gt=4).count() for i in attr_zone] for j in price_map]
                    ...
                    for idx, value in enumerate(value_zone_along_price):
                        ...
                        value_price_along_zone = [[Zufang.objects.filter(zone=j).filter(
                            price__gte=i[0]).filter(price__lt=i[1]).filter(
                            housearea__gte=int(house_args[2][0])).filter(housearea__lt=int(house_args[2][1])).filter(
                            housenum=room_map2[house_args[3]]).count() if house_args[3] != "四室以上" else
                                                       Zufang.objects.filter(zone=j).filter(
                                                           ...
                                                           housearea__lt=int(house_args[2][1])).filter(
                                                           housenum__gt=4).count()
                                                       for i in price_map] for j in attr_zone]
                    ...
                    values_zone = [
                        Zufang.objects.filter(zone=i).filter(
                            housearea__gte=int(house_args[2][0])).filter(
                            housearea__lt=int(house_args[2][1])).count() for i in attr_zone]
                    bar_zone = bar_ploy("租房区域分布图", attr_zone, values_zone, title_pos="0%")

                    values_price = [
                        Zufang.objects.filter(housearea__gte=int(house_args[2][0])).filter(
...
def gz_map():
    ...
    attr_map = ["天河区", "番禺区", "海珠区", "从化区", "增城区", "南沙区", "白云区", "花都区", "黄埔区", "越秀区", "荔湾区"]
    value_map = [Zufang.objects.filter(zone=i.strip('区')).count() for i in attr_map]
    map.add(
        "",
        attr_map,
        value_map,
